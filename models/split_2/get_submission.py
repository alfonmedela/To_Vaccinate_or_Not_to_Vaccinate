...
if __name__ == '__main__':
    ...
    train.columns = ['id', 'comment_text', 'label']
    ...
    databunch = (TextList.from_df(train_df, cols='comment_text', processor=transformer_processor)
                 .split_by_rand_pct(0.1, seed=666)
    ...
    print(learner.model)
    print(learner.model.transformer.classifier.out_proj)
    ...
    # make sure stage_final is properly generated
    learner.load('stage_final')

    test = pd.read_csv(root_path + 'Test.csv')
    sub = pd.read_csv(root_path + 'SampleSubmission.csv')

    for i in range(len(test)):
        id_sub = sub['tweet_id'][i]
        id = test['tweet_id'][i]

        if id == id_sub:
            prediction = learner.predict(test['safe_text'][i])
            prediction = prediction[-1].detach().numpy()
            prediction = prediction[0]
            print('pred', prediction)

            sub['label'][i] = str(prediction)
            print(sub['label'][i])
